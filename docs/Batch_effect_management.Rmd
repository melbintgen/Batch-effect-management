...
Batch effects refer to unwanted variation in measured data arising from biological, technical or computational factors that are not the primary focus of the study and that may obscure the biological effects of interest (e.g., treatment effects). Handling batch effects is a necessary step to improve the reproducibility of research studies. However, methods developed for batch effects often rely on strong assumptions about data characteristics and the types of batch effects.
...
**Note**: There is currently an issue with downloading *PLSDAbatch* from Bioconductor. Please try installing it directly from GitHub instead, and make sure that the installed version is later than 2.0.0.
...
We can also visualise individual variables (OTUs in this case). For example, we can select the top OTU driving the major variation in PCA using `selectVar()` in *mixOmics* package. We can then plot this OTU as boxplots and density plots using `box_plot()` and `density_plot()` in *PLSDAbatch*.
...
Linear regression will be conducted using `linear_regres()` function in *PLSDAbatch*. We integrated the *performance* package, which assesses the performance of regression models, into function `linear_regres()`. Therefore, we can apply `check_model()` from *performance* to the outputs of `linear_regres()` to diagnose the validity of models fitted with treatment and batch factors for each variable [@daniel2020performance]. 

We can also extract performance metrics such as adjusted R2, RMSE, RSE, AIC and BIC for models fitted with and without batch factors, which are included in the outputs of `linear_regres()`.
...
To assess the validity of the model fitted with both treatment and batch factors, we can use diagnostic plots to check whether the assumptions are met for each microbial variable. For example, for "OTU12", the assumptions of linearity (or homoscedasticity) and homogeneity of variance were not satisfied (top right & middle left). No sample was classified as an outlier with a Cook's distance greater than or equal to 0.9, however, samples "57", "39", "47", "44" and "16" were relatively close to the contour lines. The correlation between batch (`batch.fix`) and treatment (`trt`) factors was very low (< 5), indicating a well-fitted model with low collinearity (bottom left). The distribution of residuals was very close to normal (bottom panel). For microbial variables that violate some model assumptions, their results should be interpreted with caution. 

The `criterion` is used to determine whether the model with or without batch factors should be preferred. Here, we have selected `AIC` as the criterion. For each microbial variable, the model with the lower AIC is considered to provide the better fit and is therefore preferred.
...
We can also examine other performance metrics, such as adjusted $R^2$. However, because adjusted $R^2$ was not selected as the model-selection criterion, it is not used to choose between the models with and without batch factors.

```{r}
head(ad.lm$adj.R2)
```

The choice of the better model depends strongly on the criterion selected.

We retain only the $p$-values from the preferred model, which can be obtained as follows:
...
For the **AD data**, we apply non-parametric correction (`par.prior = FALSE`), using the input batch factor (`batch`) and the design matrix with the treatment factor (`mod`) to calculate the batch effect corrected data `ad.ComBat`. We chose the non-parametric approach based on the assumption that microbial abundance data, even after CLR transformation, may not follow a standard distribution. 
...
In PLSDA-based methods, a categorical `Y` variable is converted into a 0/1 dummy matrix, A `Y` variable with $k$ classes becomes a matrix with $k$ columns, but only $k-1$ of these are informative because the final column can always be derived from the others. Mathmetically, the rank of `Y` matrix is $k-1$.

In the **AD data**, `ad.trt` has 2 classes and `ad.batch` has 5 batches, so we choose 1 component for the treatment effect and 4 components for the batch effect.

Here, we use the default `regression` mode, which is also the recommended mode. In this application, our goal is to understand how `X` explains and predicts `Y`: specifically, to remove the variation in `X` that explains `Y.batch` while preserving the variation in `X` that explains `Y.trt`.
...
Once batch effects have been adequately removed, the corrected data can be used in downstream analyses such as dimension reduction, visualisation, clustering, network analysis, data integration and prediction. For variable selection or biomarker discovery, however, I recommend accounting for batch effects directly within the statistical model rather than first correcting the data and then performing variable selection. This two-step approach adjusts the microbial profiles but does not model the relationship between the treatment and the batch factor. When the batch and treatment factors are associated, this mismatch may create false signals or weaken true microbiome–treatment associations.
...
SVA only accounts for unknown batch effects. Therefore, we assume that the batch factor in the **AD data** is unknown, and that the batch effects are systematic. 

We first need to build two design matrices with (`ad.mod`) and without (`ad.mod0`) the treatment factor using `model.matrix()` function from *stats*. We will then use `num.sv()` from *sva* package to determine the number of surrogate or batch-related variables `n.sv` to estimate in function `sva()`.
...
The estimated batch factors are then input into `f.pvalue()` to calculate the p values of treatment effects. In SVA, batch factors are assumed to be independent of treatment factors. If they are correlated due to study design or biological reasons, the estimation of batch-related variables will be biased and may affect the accuracy of downstream linear regression results. However, some correlation may exist between the estimated batch factors and treatment due to how SVA estimates batch-related variables from those least affected by treatment. These variables may still carry some treatment variation, leading to estimated batch factors that are weakly correlated with treatment factors.  [@wang2020managing].
...
Before applying removeBatchEffect, we need to prepare a design matrix (`ad.mod`) that includes the treatment factor, which is used to preserve treatment effects during batch effect correction. This design matrix can be generated using `model.matrix()` function from *stats*. 

Then we can use `removeBatchEffect()` function (*limma* package) with the batch factor (`batch`) and the treatment design matrix (`design`) as inputs to calculate the batch effect corrected data `ad.rBE`.
...
Applying sPLSDA-batch is similar to applying PLSDA-batch. We will use the same function `PLSDA_batch()`, but need to specify the number of variables to select on each component, typically for the treatment-related components (`keepX.trt`). To determine the optimal number of variables to select, we will use `tune.splsda()` function from *mixOmics* package [@rohart2017mixomics], testing a range of possible values for each component (`test.keepX`).
...
Here, the optimal number of variables to select for the treatment component is 100. 
...
```{r}
ad.pca.before <- mixOmics::pca(ad.clr, ncomp = 3, scale = TRUE)
ad.pca.rBE <- mixOmics::pca(ad.rBE, ncomp = 3, scale = TRUE)
ad.pca.ComBat <- mixOmics::pca(ad.ComBat, ncomp = 3, scale = TRUE)
ad.pca.PLSDA_batch <- mixOmics::pca(ad.PLSDA_batch, ncomp = 3, scale = TRUE)
ad.pca.sPLSDA_batch <- mixOmics::pca(ad.sPLSDA_batch, ncomp = 3, scale = TRUE)
ad.pca.PN <- mixOmics::pca(ad.PN, ncomp = 3, scale = TRUE)
ad.pca.RUVIII <- mixOmics::pca(ad.RUVIII, ncomp = 3, scale = TRUE)
ad.pca.RUVIII.new <- mixOmics::pca(ad.RUVIII.new, ncomp = 3, scale = TRUE)
```
...
ad.pca.rBE.plot <- Scatter_Density(components = ad.pca.rBE$variates$X, 
                                   comp = c(1, 2),
                                   expl.var = ad.pca.rBE$prop_expl_var$X,
                                   batch = ad.batch, 
                                   trt = ad.trt, 
...
ad.pca.sPLSDA_batch.plot <- Scatter_Density(components = ad.pca.sPLSDA_batch$variates$X, 
                                            comp = c(1, 2),
                                            expl.var = ad.pca.sPLSDA_batch$prop_expl_var$X,
                                            batch = ad.batch, 
                                            trt = ad.trt, 
                                            title = 'sPLSDA-batch',
                                            color.set = color.mixo(1:5))

ad.pca.PN.plot <- Scatter_Density(components = ad.pca.PN$variates$X, 
                                  comp = c(1, 2),
                                  expl.var = ad.pca.PN$prop_expl_var$X,
                                  batch = ad.batch, 
                                  trt = ad.trt, 
...
ad.pca.RUVIII.new.plot <- Scatter_Density(components = ad.pca.RUVIII.new$variates$X, 
                                          comp = c(1, 2),
                                          expl.var = ad.pca.RUVIII.new$prop_expl_var$X,
                                          batch = ad.batch.new, 
                                          trt = ad.trt.new, 
                                          title = 'RUVIII+pseR',
                                          color.set = color.mixo(1:5))

...
The estimated batch effects are then input into `f.pvalue()` to calculate the p values of treatment effects. In SVA, batch effects are assumed to be independent of treatment effects. If they are correlated due to study design or biological reasons, the estimation of batch-related variables will be biased and may affect the accuracy of downstream linear regression results. However, some correlation may exist between the estimated batch effects and treatment due to how SVA estimates batch-related variables from those least affected by treatment. These variables may still carry some treatment information, leading to estimated batch effects that are weakly correlated with treatment effects.  [@wang2020managing].
